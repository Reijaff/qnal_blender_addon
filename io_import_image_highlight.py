...
def offset_planes(planes, gap, axis):
    ...
    for current in planes[1:]:
        local_offset = abs(
            (prior.dimensions + current.dimensions).dot(axis)) / 2.0 + gap
...
def auto_align_nodes(node_tree):
    ...
    def align(to_node):
        ...
        for i, node in enumerate(from_nodes):
            ...
            node.location.y += (len(from_nodes) - 1) * \
                y_gap / (len(from_nodes))
            align(node)
...
def get_shadeless_node(dest_node_tree):
    ...
    try:
        ...
        node_tree.links.new(
            camera_reflected.inputs[0], reflection_limit.outputs[0])
        ...
        node_tree.links.new(
            shadow_or_reflect.inputs[0], camera_reflected.outputs[0])
...
def find_plane_corner(object_name, x, y, axis, camera=None, *args, **kwargs):
    ...
    if args or kwargs:
        ...
        warnings.warn(
            "Unknown Parameters Passed to \"Images as Planes\".  Maybe you need to upgrade?")
    ...
    for node in node_tree.nodes.values():
        if node != frame_node and not node.parent:
            if node.location.x < newpos.x + frame_node.width:
                print("Below", node.name, node.location,
                      node.height, node.dimensions)
                newpos.y = min(newpos.y, node.location.y -
                               max(node.dimensions.y, node.height) - 20)
...
def setup_compositing(context, plane, img_spec):
    ...
    for corner in cornerpin_node.inputs[1:]:
        ...
        for i, axis_fcurve in enumerate(drivers):
            ...
            add_driver_prop(driver, 'cam_loc_x', 'OBJECT',
                            scene.camera, 'location[0]')
...
class IMPORT_IMAGE_OT_to_plane_highlight(Operator, AddObjectHelper):
    """Create mesh plane(s) from image files with the appropriate aspect ratio"""

    bl_idname = "qnal.import_image_highlight"
    bl_label = "Import Images as Planes highlight"
    bl_options = {'REGISTER', 'PRESET', 'UNDO'}

    # ----------------------
    # File dialog properties
    files: CollectionProperty(type=bpy.types.OperatorFileListElement, options={
                              'HIDDEN', 'SKIP_SAVE'})

    directory: StringProperty(maxlen=1024, subtype='FILE_PATH', options={
                              'HIDDEN', 'SKIP_SAVE'})
    ...
    offset: BoolProperty(name="Offset Planes", default=True,
    ...
    shader: EnumProperty(name="Shader", items=SHADERS,
                         default='PRINCIPLED', description="Node shader to use")
    ...
    blend_method: EnumProperty(name="Blend Mode", items=BLEND_METHODS,
                               default='BLEND', description="Blend Mode for Transparent Faces")
    ...
    shadow_method: EnumProperty(name="Shadow Mode", items=SHADOW_METHODS,
                                default='CLIP', description="Shadow mapping method")
    ...
    interpolation: EnumProperty(name="Interpolation", items=INTERPOLATION_MODES,
                                default='Linear', description="Texture interpolation")
    ...
    alpha_mode_items = tuple((e.identifier, e.name, e.description)
                             for e in t.enum_items)
    ...
    use_auto_refresh: BoolProperty(
        name=t.name, default=True, description=t.description)

    relative: BoolProperty(name="Relative Paths",
                           default=True, description="Use relative file paths")
    ...
    def execute(self, context):
        ...
        image_object = bpy.context.active_object
        image_object.scale.xyz = 11.3

        subdiv_mod_name = "Subdivision"
        subdiv_mod = image_object.modifiers.new(subdiv_mod_name, "SUBSURF")
        subdiv_mod.subdivision_type = 'SIMPLE'
        subdiv_mod.levels = 8
        bpy.ops.object.modifier_apply(modifier=subdiv_mod_name)

        geo_node_tree = bpy.data.node_groups.new(
            "ProximityDataNodeGroup.000", "GeometryNodeTree")

        output_node = geo_node_tree.nodes.new('NodeGroupOutput')
        input_node = geo_node_tree.nodes.new('NodeGroupInput')
        ...
        geo_node_tree.links.new(
            output_node.inputs[0], named_attr_node.outputs[0])

        collection_info_node = geo_node_tree.nodes.new(
            "GeometryNodeCollectionInfo")
        collection_info_node.transform_space = "RELATIVE"
        # collection_info_node.inputs[1].default_value = True

        realize_instances_node = geo_node_tree.nodes.new(
            "GeometryNodeRealizeInstances")

        geo_node_tree.links.new(
            collection_info_node.outputs[0], realize_instances_node.inputs[0])

        proximity_node = geo_node_tree.nodes.new("GeometryNodeProximity")
        proximity_node.target_element = 'FACES'

        geo_node_tree.links.new(
            realize_instances_node.outputs[0], proximity_node.inputs[0])

        map_range_node = geo_node_tree.nodes.new("ShaderNodeMapRange")
        ...
        map_range_node.inputs[2].default_value = 0.002
        map_range_node.inputs[3].default_value = 2.5
        map_range_node.inputs[4].default_value = 0
        ...
        gmn = image_object.modifiers.new("ProximityData", "NODES")
        gmn.node_group = geo_node_tree

        shader_node_tree = image_object.active_material.node_tree
        for node in shader_node_tree.nodes:
            if node.name == "Image Texture" or node.name == "Material Output":
                continue
            shader_node_tree.nodes.remove(node)

        final_input_socket = shader_node_tree.nodes["Material Output"].inputs[0]
        start_output_socket = shader_node_tree.nodes["Image Texture"].outputs[0]

        ih_group_node_tree = bpy.data.node_groups.new(
            'inverse_highlight', 'ShaderNodeTree')

        ih_output_node = ih_group_node_tree.nodes.new('NodeGroupOutput')
        ih_input_node = ih_group_node_tree.nodes.new('NodeGroupInput')

        ih_group_node_tree.inputs.new('NodeSocketColor', 'Image')
        ih_group_node_tree.inputs.new('NodeSocketFloat', 'Exponent')
        ih_group_node_tree.inputs.new('NodeSocketColor', 'white')
        ih_group_node_tree.inputs.new('NodeSocketColor', 'black')
        ih_group_node_tree.outputs.new('NodeSocketColor', 'Color')

        attr_node = ih_group_node_tree.nodes.new("ShaderNodeAttribute")
        attr_node.attribute_name = "mask"

        invert_node = ih_group_node_tree.nodes.new("ShaderNodeInvert")
        ih_group_node_tree.links.new(
            ih_input_node.outputs[0], invert_node.inputs[1])

        mix_node = ih_group_node_tree.nodes.new("ShaderNodeMix")
        mix_node.data_type = 'RGBA'
        ih_group_node_tree.links.new(attr_node.outputs[0], mix_node.inputs[0])
        ih_group_node_tree.links.new(
            ih_input_node.outputs[0], mix_node.inputs[6])
        ih_group_node_tree.links.new(
            mix_node.outputs[2], ih_output_node.inputs[0])

        power_node = ih_group_node_tree.nodes.new("ShaderNodeMath")
        power_node.operation = 'POWER'
        power_node.inputs[1].default_value = 8

        ih_group_node_tree.links.new(
            ih_input_node.outputs[1], power_node.inputs[1])
        ih_group_node_tree.links.new(
            invert_node.outputs[0], power_node.inputs[0])

        shader_map_range_node = ih_group_node_tree.nodes.new(
            "ShaderNodeMapRange")
        shader_map_range_node.data_type = 'FLOAT_VECTOR'
        shader_map_range_node.inputs[8].default_value = (0.5, 0.5, 0.5)

        ih_group_node_tree.links.new(
            shader_map_range_node.outputs[1], mix_node.inputs[7])
        ih_group_node_tree.links.new(
            power_node.outputs[0], shader_map_range_node.inputs[6])
        ih_group_node_tree.links.new(
            ih_input_node.outputs[2], shader_map_range_node.inputs[9])
        ih_group_node_tree.links.new(
            ih_input_node.outputs[3], shader_map_range_node.inputs[10])

        auto_align_nodes(ih_group_node_tree)

        group_node = shader_node_tree.nodes.new("ShaderNodeGroup")
        group_node.node_tree = ih_group_node_tree

        shader_node_tree.links.new(group_node.inputs[0], start_output_socket)
        shader_node_tree.links.new(group_node.outputs[0], final_input_socket)

        group_node.inputs["Exponent"].default_value = 2
        group_node.inputs["white"].default_value = (0, 0, 0, 1)
        group_node.inputs["black"].default_value = (1, 1, 0, 1)

        auto_align_nodes(shader_node_tree)

        imported_image_collection = bpy.data.collections.new(
            image_object.name_full)
        bpy.context.scene.collection.children.link(imported_image_collection)
        image_object.users_collection[0].objects.unlink(image_object)
        imported_image_collection.objects.link(image_object)

        highlights_collection = bpy.data.collections.new(
        ...
        collection_info_node.inputs[0].default_value = highlights_collection
    ...
    def import_images(self, context):
        ...
        planes = [self.single_image_spec_to_plane(
            context, img_spec) for img_spec in images]
    ...
    def create_cycles_material(self, context, img_spec):
        ...
        if self.shader in {'PRINCIPLED', 'SHADELESS'}:
            node_tree.links.new(
                core_shader.inputs[0], tex_image.outputs['Color'])
        elif self.shader == 'EMISSION':
            node_tree.links.new(
                core_shader.inputs['Emission'], tex_image.outputs['Color'])

        if self.use_transparency:
            if self.shader in {'PRINCIPLED', 'EMISSION'}:
                node_tree.links.new(
                    core_shader.inputs['Alpha'], tex_image.outputs['Alpha'])
            else:
                bsdf_transparent = node_tree.nodes.new(
                    'ShaderNodeBsdfTransparent')

                mix_shader = node_tree.nodes.new('ShaderNodeMixShader')
                node_tree.links.new(
                    mix_shader.inputs['Fac'], tex_image.outputs['Alpha'])
                node_tree.links.new(
                    mix_shader.inputs[1], bsdf_transparent.outputs['BSDF'])
                node_tree.links.new(
                    mix_shader.inputs[2], core_shader.outputs[0])
    ...
    def create_image_plane(self, context, name, img_spec):
        ...
        if self.size_mode == 'CAMERA':
            ...
            center_in_camera(
                context.scene, context.scene.camera, plane, translate_axis)
...
def import_images_highlight_button(self, context):
    self.layout.operator(IMPORT_IMAGE_OT_to_plane_highlight.bl_idname,
                         text="Images as Planes highlight", icon='TEXTURE')


# classes = (
    # IMPORT_IMAGE_OT_to_plane_highlight,
# )
...
# def register():
    ...
    # bpy.types.TOPBAR_MT_file_import.append(import_images_highlight_button)
    # bpy.types.VIEW3D_MT_image_add.append(import_images_highlight_button)
...
# def unregister():
    # bpy.types.TOPBAR_MT_file_import.remove(import_images_highlight_button)
    # bpy.types.VIEW3D_MT_image_add.remove(import_images_highlight_button)
